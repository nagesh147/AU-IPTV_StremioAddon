...
# --- patched helpers (quote-aware) ---
_attr_pair_re = re.compile(r'([A-Za-z0-9_-]+)=(?:"([^"]*)"|\'([^\']*)\'|([^,\s]+))')

def _split_extinf_header_display(line: str):
    """
    Split the EXTINF header from its display title at the *first comma that is not inside quotes*.
    Returns (header, display). If no display is present, display is "".
    """
    in_quote = False
    q = None
...
def _parse_attr_pairs(attr_str: str) -> dict:
    """
    Parse key=value pairs in an EXTINF header, respecting quoted values that may contain
    commas or spaces. Accepts both hyphenated and underscored keys.
    """
    attrs = {}
    for m in _attr_pair_re.finditer(attr_str or ""):
        key = m.group(1)
...
# --- override with quote-aware version ---
def parse_m3u_entries(text: str):
    """Yield dicts with id, name, logo, group, url for each #EXTINF block (first URL)."""
    meta = None
    for raw in (text or '').splitlines():
        ...
        if not line or line.startswith('#EXTM3U'):
            continue
        if line.startswith('#EXTINF'):
            # Split header vs display at the first comma *outside quotes*
            header, disp = _split_extinf_header_display(line)
            # Grab the attribute part (after the first space following #EXTINF:-1)
            if ' ' in header:
            ...
            # Robust logo extraction (fall back to regex only if the attribute wasn't found)
            logo = attrs.get('tvg-logo') or attrs.get('logo')
            if not logo:
                m = re.search(r'(?:tvg-logo|logo)\s*=\s*"?(https?://\S+?)"?(?:\s|$|,)', line, re.I)
                if m: logo = m.group(1).rstrip('",')
            ...
            name = (attrs.get('tvg-name') or (disp or "").strip() or attrs.get('tvg-id') or "").strip()
            # Guard against accidental URL fragments leaking into 'name'
            if 'http://' in name or 'https://' in name:
                name = name.split('http://')[0].split('https://')[0].strip(' "')
            gid = attrs.get('tvg-id') or name or None
            group = attrs.get('group-title') or None
...
# --- final override of _parse_attr_pairs ---
_attr_pair_re = re.compile(r'([A-Za-z0-9_-]+)=(?:"([^"]*)"|' + "'" + r'([^\']*)' + "'" + r'|([^,\s]+))')
def _parse_attr_pairs(attr_str: str) -> dict:
    attrs = {}
    for m in _attr_pair_re.finditer(attr_str or ""):
        key = m.group(1)
