  ...
  builder.defineCatalogHandler(async ({ type, id, extra }) => {
    ...
    if (contentKind === 'tv' && (isCurated || isExtras)) {
      ...
      rogueIdx = cache.rogue_epg_idx.get(`rogue:${regionKey}`)
            || await getOrBuildRogueIndex(regionKey, 60000); // Increased from 8000 to 15000ms
    }
...
builder.defineMetaHandler(async ({ type, id }) => {
  ...
  try {
    if (region === 'NZ') {
      ...
      const idx = cache.rogue_epg_idx.get(`rogue:${regionKeyForScope}`)
        || await getOrBuildRogueIndex(regionKeyForScope, 60000);
      if (idx) {
